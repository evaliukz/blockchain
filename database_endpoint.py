...
# Takes input dictionary d and writes it to the Log table actually should be the JSON of the dict, not the dict
...
def trade():
    if request.method == "POST":
        ...
        if not platform in platforms:
            print("Error: platform NOT Algorand or Ethereum")
            return jsonify(False)

        # Your code should check whether “sig” is a valid signature of 
        # json.dumps(payload), using the signature algorithm specified by the platform field. Be sure to verify the payload 
        # using the sender_pk.
        result = False
       ...
       # Instead, insert a record into the “Log” table, with the message field set to be json.dumps(payload).
        if result is False:
            print("Signature does NOT verified!")
...
def order_book():
    ...
    # convert orders from DB into a list
    order_dict_list = [
           convertToDictionary(order)
           for order in g.session.query(Order).all()
    ]
        
    # append the list into a dictionary
    result = {
